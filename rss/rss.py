...
class RSS(object):
    ...
    async def valid_url(self, url):
        ...
        # validation disabled cuz i work with 'bozo' feeds
        # if rss.bozo:
        #     return False
        # else:
        #     return True
    ...
    async def _rss_force(self, ctx, feed_name: str):
        ...
        messages = await self.get_current_feed(server.id, channel.id,
                                              feed_name, items)
    ...
    async def get_current_feed(self, server, chan_id, name, items):
        ...
        if not items['last']:
            ...
            self.feeds.update_time(
                server, chan_id, name, items['last'])
        ...
        for entry in rss.entries:
            try:
                published = time.mktime(time.strptime(entry.published, date_template))
            except IndexError:
            ...
            if published > last_time:
                ...
                try:
                    summary_escaped = re.sub("<br? ?/>", "\n", entry.summary)
                    summary_escaped = re.sub("<[^<]+?>", "", summary_escaped)
                    summary_escaped = re.sub(" +", " ", summary_escaped)
                except IndexError:
                    summary_escaped = ''

                to_fill = string.Template(template)
                entry_list.append(to_fill.safe_substitute(
                    name=bold(name),
        ...
        if len(entry_list):
            try:
                self.feeds.update_time(
                    server, chan_id, name, time.mktime(time.strptime(rss.entries[0].published, date_template)))
            except IndexError:
                return []
                ...
                for chan_id in feeds[server]:
                    for name, items in feeds[server][chan_id].items():
                        ...
                        if channel is None:
                            ...
                            continue
                        msgs = await self.get_current_feed(server, chan_id,
                                                          name, items)
                        for msg in msgs:
                            if msg is not None:
